...
many <- modleR::do_many(species_name = species[1], predictors = example_vars, 
    models_dir = test_folder, png_partitions = F, write_bin_cut = FALSE, 
    write_rda = TRUE, bioclim = FALSE, domain = FALSE, glm = FALSE, 
    svmk = FALSE, svme = FALSE, maxnet = F, rf = F, mahal = FALSE, 
    brt = TRUE, equalize = TRUE)
fm <- modleR::final_model(species_name = species[1], algorithms = NULL, 
    models_dir = test_folder, which_models = c("raw_mean", "bin_mean", 
        "bin_consensus"), consensus_level = 0.5, uncertainty = TRUE, 
